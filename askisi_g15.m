...
% Συχνότητα δειγματοληψίας για x3
fs3 = 250; % Hz
Ts3 = 1/fs3; % Περίοδος δειγματοληψίας
...
% Υπολογισμός των δειγμάτων του σήματος για fs3
Xs3 = cos(100*pi*N1*Ts3) + cos(200*pi*N1*Ts3) + sin(500*pi*N1*Ts3);

% Ανακατασκευή του σήματος x3 χρησιμοποιώντας το θεώρημα δειγματοληψίας για fs3
x3 = zeros(1, length(t));
for k = 1:length(t)
    x3(k) = Xs3 * sinc((t(k) - N1*Ts3) / Ts3)';
end
...
% Σχεδίαση του ανακατασκευασμένου σήματος x3
plot(t, x3, 'm--', 'LineWidth', 1.5);

% Προσθήκη υπομνήματος (legend) για τα σήματα
legend('Αρχικό σήμα x(t)', 'Ανακατασκευασμένο σήμα x1(t) (fs = 1000 Hz)', 'Ανακατασκευασμένο σήμα x2(t) (fs = 500 Hz)', 'Ανακατασκευασμένο σήμα x3(t) (fs = 250 Hz)');

% Ορισμός του τίτλου του διαγράμματος
title('Ανακατασκευή Σήματος με Θεώρημα Δειγματοληψίας (fs = 1000 Hz, fs = 500 Hz και fs = 250 Hz)');
